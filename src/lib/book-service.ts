...
export async function listPublishedBooks({
  ...
  const [books, total] = await Promise.all([
    BookModel.find(filter).sort({ publishedAt: -1 }).skip(skip).limit(limit).lean(),
...
export async function getChapterBySlugs(bookSlug: string, chapterSlug: string) {
  await connectToDatabase();
  const book = await BookModel.findOne({ slug: bookSlug, visibility: 'public' }).lean();
  if (!book) return null;
  const chapter = await ChapterModel.findOne({ bookId: book._id, slug: chapterSlug }).lean();
