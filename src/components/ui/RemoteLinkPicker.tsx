...
export default function RemoteLinkPicker({ open, onClose, onSelect, type = 'image', title = 'Select Remote Link' }: RemoteLinkPickerProps) {
  ...
  const fetchLinks = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/assets/links?${params.toString()}`, { credentials: 'include', signal });
          ...
          <Tabs value={activeType} onValueChange={(v) => setActiveType(v as any)} className="w-full">
            ...
            <TabsContent value={activeType} className="m-0">
              ...
              {loading ? (
                ...
                <div className="py-10 text-center text-sm opacity-70">No links found</div>
              ) : (
                    ...
                    >
                      <div className="aspect-video bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden">
                        {link.type === 'image' ? (
                          ...
                          <img src={link.thumbnailUrl || link.url} alt={link.name} className="w-full h-full object-cover" />
                        ) : (
                          <div className="text-xs opacity-70 p-2 text-center">{link.type.toUpperCase()}</div>
                        )}
                      </div>
                      <div className="p-2 text-left">
                        <div className="text-xs font-medium truncate" title={link.name}>{link.name}</div>
                        {link.mimeType && <div className="text-[10px] text-gray-500 truncate">{link.mimeType}</div>}
                      </div>
                    </button>
                  ))}
                </div>
