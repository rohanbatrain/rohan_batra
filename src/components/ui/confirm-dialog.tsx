    ...
    <Dialog open={state.open} onOpenChange={(open) => { if (!open) handleClose(); }}>
      <DialogContent>
        ...
        {state.options.description ? (
          <div className="text-sm text-muted-foreground">{state.options.description}</div>
        ) : null}
        <DialogFooter>
          <Button variant="ghost" onClick={handleClose}>
            {state.options.cancelText || 'Cancel'}
          </Button>
          <Button onClick={handleConfirm} variant={state.options.destructive ? 'destructive' : 'default'}>
