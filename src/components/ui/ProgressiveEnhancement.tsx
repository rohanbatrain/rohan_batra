        ...
        try {
          ...
          if (config.advancedFeatures) {
            for (const feature of config.advancedFeatures) {
              const featureKey = feature.startsWith('advanced.')
                ? feature
                : `advanced.${feature}`;
          ...
          if (
            !hasAllRequired ||
            (!hasAnyAdvanced && config.advancedFeatures?.length)
          ) {
          ...
          <div className='flex items-center gap-2 flex-wrap'>
            ...
            {Object.entries(availableFeatures).map(([feature, enabled]) => (
              <Badge
                ...
                className={
                  enabled ? 'bg-blue-100 text-blue-800' : 'text-gray-500'
                }
              >
    ...
    const checkFeatures = async () => {
      try {
        ...
        const enabled = features.filter(
          feature => allFeatures[feature]?.enabled ?? false
        );
