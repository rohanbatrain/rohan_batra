      ...
      <div className='mx-auto max-w-6xl'>
        ...
        >
          ...
          <p className='text-gray-600 dark:text-gray-300 max-w-3xl mx-auto'>
            Specializing in web, mobile, security, cloud, and platform
            engineering with a focus on performance, reliability, and user
            experience.
          </p>
        </motion.div>
        ...
        <Tabs defaultValue='all' className='space-y-6'>
          ...
          {[{ key: 'all', label: 'All' } as any, ...skillCategories].map(
            cat => {
              const allItems =
                cat.key === 'all'
                  ? skillsData
                  : skillsData.filter(s => s.category === cat.key);
              const items =
                cat.key === 'all' && !showAllExpanded
                  ? allItems.slice(0, 6)
                  : allItems;
              return (
                      ...
                      (
                        ...
                        >
                          ...
                          <div className='flex items-center justify-center gap-3'>
                            <Link
                              href={`/blog?tag=${encodeURIComponent((blogTags[0] || title).toLowerCase())}`}
                            ...
                            <Link
                              href={`/portfolio?tag=${encodeURIComponent((projectTags[0] || title).toLowerCase())}`}
                              className='text-xs text-primary hover:underline'
                            >
                              Projects
                            </Link>
