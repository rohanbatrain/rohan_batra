...
describe('GET /api/blog/posts/[slug]', () => {
  it('should return a single blog post by slug', async () => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/blog/posts/${slug}`
    );
  ...
  it('should return 404 for non-existent slug', async () => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/blog/posts/${nonExistentSlug}`
    );
  ...
  it('should handle special characters in slug', async () => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/blog/posts/${slugWithSpecialChars}`
    );
    const data = await response.json();
  ...
  it('should return post with related posts', async () => {
    ...
    const response = await fetch(
      `http://localhost:3000/api/blog/posts/${slug}?include=related`
    );
    const data = await response.json();
