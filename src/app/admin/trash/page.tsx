...
export default function TrashPage() {
  ...
  const act = async (
    ...
    try {
      ...
      setProcessingIds(prev => ({
        ...prev,
        [type]: new Set([...(prev[type] ? Array.from(prev[type]) : []), id]),
      }));
      ...
      setProcessingIds(prev => {
        const next = { ...prev };
        const s = new Set(next[type] ? Array.from(next[type]) : []);
        s.delete(id);
        next[type] = s;
        return next;
      });
    ...
    try {
      for (const [type, ids] of entries) {
        ...
        setProcessingIds(prev => ({
          ...prev,
          [type]: new Set([
            ...(prev[type] ? Array.from(prev[type]) : []),
            ...idList,
          ]),
        }));
        ...
        const res = await fetch('/api/admin/trash', {
          ...
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ type: type, ids: idList, action }),
        });
        ...
        if (!res.ok || body?.success === false) {
          toast({
            title: 'Error',
            description: body?.error || 'Bulk operation failed',
            variant: 'destructive',
          });
          ...
          for (const r of body.results) {
            if (r.success) {
              toast({
                title: action === 'restore' ? 'Restored' : 'Deleted',
                description: `Item ${r.id}: ${r.message || 'OK'}`,
              });
            } else {
              toast({
                title: 'Error',
                description: `Item ${r.id}: ${r.message || 'Failed'}`,
                variant: 'destructive',
              });
            }
          }
        ...
        // clear processed ids for this type
        setProcessingIds(prev => {
          const next = { ...prev };
          next[type] = new Set();
          return next;
      ...
      toast({
        title: 'Error',
        description: e instanceof Error ? e.message : 'Bulk failed',
        variant: 'destructive',
      });
    } finally {
  ...
  const toggle = (type: EntityType, id: unknown) => {
    const sid = String(id);
    setSelected(prev => {
      const next: Record<string, Set<string>> = { ...prev } as Record<
        string,
        Set<string>
      >;
    ...
    <div className='space-y-4'>
          ...
          <button
            ...
            onClick={async () => {
              const ok = await confirm({
                title: 'Empty Trash',
                description:
                  'Empty trash for all types? This permanently deletes these items.',
                confirmText: 'Empty',
                cancelText: 'Cancel',
                destructive: true,
              });
              ...
              try {
                ...
                toast({
                  title: 'Deleted',
                  description: 'Perma-deleted all trashed items',
                });
                await mutate();
              } catch (e) {
                toast({
      ...
      {types
        ...
        .map(t => {
          ...
          return (
            <div key={String(t)} className='space-y-2'>
                  ...
                  >
                    <label className='flex items-center gap-2'>
                      <input
                        type='checkbox'
                        checked={
                          !!selected[t as string]?.has(
                            String(it?._id ?? it?.id)
                          )
                        }
                        onChange={() =>
                          toggle(t as EntityType, it?._id ?? it?.id)
                        }
