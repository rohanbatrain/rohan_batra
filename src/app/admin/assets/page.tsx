...
export default function AssetsManagementPage() {
  ...
  const [activeTab, setActiveTab] = useState<'lottie' | 'images' | 'links'>(
    'lottie'
  );
  ...
  const fetchAssets = useCallback(async () => {
    ...
    try {
      ...
      const assetsArray = (result?.data?.assets ??
        result?.assets ??
        []) as LottieAsset[];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch('/api/admin/assets?type=image&limit=50', {
          credentials: 'include',
        });
        if (res.ok) {
          const body = await res.json();
  ...
  const deleteAsset = async (assetId: string) => {
    ...
    try {
      ...
      toast({
        title: 'Deleted',
        description: result?.message || 'Asset deleted',
      });
  ...
  const moveToTrash = async (assetId: string) => {
    if (!confirm('Move this asset to trash? You can permanently delete later.'))
      return;
    try {
      ...
      toast({
        title: 'Trashed',
        description: data.message || 'Moved to trash',
      });
      await fetchAssets();
    } catch (e) {
      toast({
  ...
  const importFromUrl = async () => {
    ...
    try {
      ...
      toast({
        title: 'Imported',
        description: data.message || 'Lottie imported',
      });
      setImportUrl('');
      fetchAssets();
    } catch (e) {
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      toast({
        title: 'Uploaded',
        description: result?.message || 'Upload complete',
      });
  ...
  const addLink = async () => {
    try {
      ...
      const refreshed = await fetch('/api/admin/assets/links', {
        credentials: 'include',
      }).then(r => r.json());
      ...
      toast({
        title: 'Error',
        description: e instanceof Error ? e.message : 'Failed to add link',
        variant: 'destructive',
      });
    }
  };
  ...
  const deleteLink = async (id: string) => {
    try {
      const res = await fetch(`/api/admin/assets/links?id=${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const body = await res.json();
      ...
      toast({
        title: 'Error',
        description: e instanceof Error ? e.message : 'Failed to delete link',
        variant: 'destructive',
      });
    }
  };
        ...
        <input
          ...
          onChange={async e => {
            ...
            try {
              ...
              toast({
                title: 'Uploaded',
                description: body?.data ? 'Image uploaded' : 'Upload complete',
              });
              // refresh images list
              const refreshed = await fetch(
                '/api/admin/assets?type=image&limit=50',
                { credentials: 'include' }
              );
              const refreshedBody = await refreshed.json().catch(() => ({}));
              if (refreshed.ok)
                setImageAssets(
                  (refreshedBody?.data?.assets || []) as ImageAsset[]
                );
          ...
          <div className='bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 space-y-4'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-5 gap-2'>
              <select
                ...
                value={newLink.type}
                onChange={e =>
                  setNewLink(v => ({ ...v, type: e.target.value }))
                }
              >
              ...
              <Input
                ...
                value={newLink.name}
                onChange={e =>
                  setNewLink(v => ({ ...v, name: e.target.value }))
                }
              />
              <Input
                placeholder='https://...'
              ...
              <Input
                ...
                onChange={e =>
                  setNewLink(v => ({ ...v, thumbnailUrl: e.target.value }))
                }
              />
              <Input
                ...
                onChange={e =>
                  setNewLink(v => ({ ...v, description: e.target.value }))
                }
              />
            </div>
            <div className='flex justify-end gap-2'>
      ...
      {activeTab === 'links' ? (
              ...
              .map(link => (
                <Card key={link._id} className='overflow-hidden'>
                  <div className='aspect-video bg-gray-50 flex items-center justify-center'>
                    ...
                    {link.type === 'other' && (
                      <div className='text-gray-500 text-sm p-4 break-all'>
                        {link.url}
                      </div>
                    )}
                  </div>
                  ...
                  <CardContent className='space-y-3'>
                    <div className='flex items-center justify-between gap-2'>
                      <a
                        ...
                        target='_blank'
                        rel='noreferrer'
                        className='flex items-center gap-2 min-w-0 truncate text-blue-600 hover:underline'
                      >
                        {}
                        <img
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          ...
          {imageAssets.length === 0 && (
            <div className='col-span-full text-center py-8 text-gray-500'>
              No images yet. Use Add Asset → Upload Image.
            </div>
          )}
        </div>
      ) : loading ? (
