...
export default function AdminJournalManager({
  ...
  useEffect(() => {
    (async () => setP(await params))();
  }, [params]);
  const { data: vol, mutate: mutateVol } = useSWR(
    () => (p ? `/api/admin/journal-volumes/${p.journalId}` : null),
    fetcher
  );
  const { data: entries, mutate } = useSWR(
    () =>
      p
        ? `/api/admin/characters/${p.id}/journals?journalId=${p.journalId}`
        : null,
    fetcher
  );
  const { data: tzSetting } = useSWR(
    '/api/admin/settings/site.timezone',
    fetcher
  );
  ...
  function toIsoFromDateTime(dateStr?: string, timeStr?: string) {
    /* ...existing code... */ return undefined as any;
  }
  ...
  async function createEntry() {
    ...
    const res = await fetch(`/api/admin/characters/${p.id}/journals`, {
      method: 'POST',
    ...
    if (res.ok) {
      ...
      toast({
        title: 'Error',
        description: err?.error || 'Failed to create entry',
        variant: 'destructive',
      });
    }
  }
  ...
  async function createQuickNote() {
    ...
    if (res.ok) {
      ...
      toast({
        title: 'Error',
        description: err?.error || 'Failed to create quick note',
        variant: 'destructive',
      });
    }
  }
  ...
  async function saveEdit() {
    ...
    const res = await fetch(`/api/admin/journals/${editing.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
      ...
      toast({
        title: 'Error',
        description: err?.error || 'Failed to update',
        variant: 'destructive',
      });
    }
  }
  ...
  async function togglePublish(j: any) {
    ...
    if (res.ok) mutate();
      ...
      toast({
        title: 'Error',
        description: 'Failed to update publish',
        variant: 'destructive',
      });
  }
  ...
  async function trashEntry(entryId: string) {
    const ok = confirm('Move this entry to trash?');
    if (!ok) return;
    const res = await fetch(`/api/admin/journals/${entryId}?trash=true`, {
      method: 'DELETE',
    });
    if (res.ok) {
      mutate();
    ...
    <div className='p-6 space-y-6'>
      ...
      <Card className='p-0 overflow-hidden'>
        <div className='p-4 flex flex-row flex-wrap items-center gap-2'>
          ...
          <InfoTip
            label={`Instant entry with current ${tzSetting?.setting?.value || 'UTC'} time and auto title (yyyy-mm-dd-hh-mm-ss).`}
          >
            <Button variant='outline' onClick={createQuickNote}>
              Quick Note (Now)
            </Button>
          </InfoTip>
          <InfoTip label='Create a draft entry with the given title.'>
            <Button onClick={createEntry} disabled={creating || !newTitle}>
              Create
            </Button>
          </InfoTip>
        </div>
                ...
                <Fragment key={`grp-${k}`}>
                  ...
                  {grouped.map[k].map((j: any) => (
                    <TableRow key={j.id}>
                      ...
                      <TableCell>
                        {j.publishedAt
                          ? new Date(j.publishedAt).toLocaleString()
                          : '-'}
                      </TableCell>
                      <TableCell>
                        <Switch
                          checked={!!j.isPrivate}
                          onChange={e =>
                            togglePrivate(j, e.currentTarget.checked)
                          }
                        />
                      </TableCell>
      ...
      <Dialog open={volEditOpen} onOpenChange={setVolEditOpen}>
        <DialogContent>
          ...
          <div className='space-y-3'>
            ...
            <div className='flex items-center gap-6 pt-1'>
              ...
              <div className='flex items-center gap-2'>
                <Switch
                  checked={volPrivate}
                  onChange={e => setVolPrivate(e.currentTarget.checked)}
                />
          ...
          <DialogFooter>
            <Button variant='ghost' onClick={() => setVolEditOpen(false)}>
              Cancel
            </Button>
            <Button
              onClick={async () => {
                ...
                setVolSaving(true);
                const payload: any = {
                  title: volTitle || undefined,
                  description: volDescription,
                  isPrivate: volPrivate,
                  status: volStatus,
                };
                const res = await fetch(
                  `/api/admin/journal-volumes/${p.journalId}`,
                  {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
