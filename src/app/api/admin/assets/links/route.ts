...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Failed to list links' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    let { type, name, url, thumbnailUrl, description, tags } = body || {};
    if (!name || !url) return NextResponse.json({ success: false, error: 'name and url are required' }, { status: 400 });
    ...
    if (!type) {
      ...
      if (mimeType?.startsWith('image/') || ['png','jpg','jpeg','gif','webp','svg','bmp'].includes(ext || '')) type = 'image';
      else if (mimeType?.startsWith('video/') || ['mp4','mov','webm','avi','mkv'].includes(ext || '')) type = 'video';
      else type = 'other';
    }

    if (!thumbnailUrl && type === 'image') thumbnailUrl = url; // simple default

    const doc = await AssetLink.create({ type, name, url, thumbnailUrl, description, tags, mimeType, size });
