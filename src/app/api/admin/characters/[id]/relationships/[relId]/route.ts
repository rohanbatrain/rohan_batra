...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string; relId: string }> }) {
  try {
    ...
    if (data.relationshipType !== undefined) rel.relationshipType = data.relationshipType;
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string; relId: string }> }) {
  try {
    const { userId } = await auth();
    if (!userId) return NextResponse.json({ success: false, error: 'Auth required' }, { status: 401 });
    await connectToDatabase();
    ...
    if (removeReciprocal) {
      ...
      if (other) {
        const match = other.relationships.find((r: any) => r.characterId.toString() === id);
        if (match) {
          match.deleteOne();
          await other.save();
        }
    ...
    return NextResponse.json({ success: false, error: 'Failed to delete relationship' }, { status: 500 });
  }
}
