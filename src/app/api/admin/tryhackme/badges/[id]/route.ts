...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const update: any = {};
  for (const key of ['title','thmBadgeId','imageUrl','link','description','category','tags','earnedAt','visibility']) {
    if (key in body) update[key] = body[key];
  }
  const updated = await TryHackMeBadgeModel.findByIdAndUpdate(params.id, update, { new: true });
  if (!updated) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  return NextResponse.json({ badge: { ...(updated.toJSON() as any), _id: updated._id.toString() } });
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!authz.ok) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  await connectToDatabase();
  const res = await TryHackMeBadgeModel.deleteOne({ _id: params.id });
