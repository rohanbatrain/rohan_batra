  ...
  if (userRole === 'editor' && currentUser?._id) bookFilter.authorId = currentUser._id;
    ...
    for (const item of data.order) {
      if (numbers.has(item.chapterNumber)) {
        return NextResponse.json({ error: 'Duplicate chapter numbers' }, { status: 400 });
      }
    ...
    const chapters = await ChapterModel.find({ _id: { $in: ids }, bookId }).select('_id');
    if (chapters.length !== ids.length) {
      return NextResponse.json({ error: 'One or more chapters not found in this book' }, { status: 400 });
    }

    // Apply updates
    await Promise.all(
      data.order.map(item =>
        ChapterModel.updateOne({ _id: item.chapterId, bookId }, { $set: { orderIndex: item.chapterNumber, updatedAt: new Date() } })
      )
    );
