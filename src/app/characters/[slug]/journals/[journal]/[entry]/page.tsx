...
interface Params {
  params: Promise<{ slug: string; journal: string; entry: string }>;
}
...
export default async function CharacterJournalEntryPage({ params }: Params) {
  ...
  const doc = (await CharacterJournal.findOne({
    characterId: character._id,
    journalId: vol._id,
    slug: entry,
    status: 'published',
  ...
  return (
    <main className='container mx-auto px-4 py-10'>
      <div className='mb-6'>
        <a
          href={`/characters/${slug}/journals/${journal}`}
          className='text-sm text-blue-600 hover:underline'
        >
          &larr; Back to {vol.title}
        </a>
      </div>
      <h1 className='text-3xl font-bold'>{doc.title}</h1>
      {doc.publishedAt && (
        <p className='text-sm text-muted-foreground mt-1'>
          {new Date(doc.publishedAt as any).toLocaleString()}
        </p>
      )}
      <article
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/characters/${slug}/journals/${journal}/${entry}`,
    },
  };
}
