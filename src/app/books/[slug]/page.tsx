  ...
  return (
    <main className='container mx-auto px-4 py-10'>
      ...
      {(book as any).description && (
        <p className='text-gray-700 mb-6'>{(book as any).description}</p>
      )}
            ...
            {chapters.map((c: any) => (
              <li key={c._id}>
                <a
                  ...
                  href={`/books/${(book as any).slug}/${c.slug}`}
                >
                  {c.title}
                </a>
              </li>
          ...
          <ul className='grid sm:grid-cols-2 gap-3'>
            {characters.map((ch: any) => (
              <li key={ch._id} className='border rounded p-3'>
                <a className='hover:underline' href={`/characters/${ch.slug}`}>
                  {ch.name}
                </a>
                {ch.role && (
